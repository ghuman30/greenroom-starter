...
| T4 | Wednesday pre-flight (`/preflight`) | **DONE** | Server component reads un-filtered DB (NOT `getReports()` — that has the D18 past-only bug). 8 signals computed per upcoming show (60-day horizon). Risk scoring + agent-posture modifier from `preferences_notes`. Demo state: Sunday Drivers 2026-07-01 (hidden dispute) and House of Lights surface in top 3 with tier_ratchet flags. Q14 backtest with LLM signal (i): at 2% extraction coverage, LLM matches regex recall while trimming 2 false positives (113→111). Full-coverage projection deferred to honest memo disclosure. kieran-typescript-reviewer flagged 4 bugs (sentinel sql, lte self-inclusion, JSON shape cast, over-broad recoup keywords) + 2 minor — all fixed. |
